...
export function Layout({ children, type }: LayoutProps) {
  ...
  // Agency portal with vertical sidebar
  if (type === 'agency') {
    return (
      <div className="min-h-screen bg-gray-50 flex">
        ...
        <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
          ...
          <div className="h-16 flex items-center px-6 border-b border-gray-200">
            <Link href="/agency" className="text-xl font-bold text-primary-600">
              Uptnable
            </Link>
          </div>
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            {navItems.map((item) => {
              ...
              // For dashboard (/agency), only match exactly
          ...
          <div className="border-t border-gray-200 p-4">
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center px-3 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none"
            >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto py-6 px-6 lg:px-8">
            {children}
          </main>
        </div>
      </div>
      ...
      >
        ...
        <nav className="flex-1 px-3 py-2 overflow-y-auto overflow-x-hidden min-h-0">
          <ul className="space-y-0.5">
            {navItems.map((item) => {
              const Icon = item.icon
              // For dashboard (/client), only match exactly
              // For other pages, match if pathname starts with the href
              ...
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    onClick={() => {
                      // Only close sidebar on mobile, don't auto-open if closed
                      if (window.innerWidth < 768 && isSidebarOpen) {
                        setIsSidebarOpen(false)
                      }
                    }}
