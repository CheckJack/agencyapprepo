...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Verify all campaigns belong to the client
    ...
    if (campaigns.length !== ids.length) {
      return NextResponse.json({ error: 'Some campaigns not found or unauthorized' }, { status: 403 })
    }
    ...
    return NextResponse.json({ 
      success: true, 
      message: `${action === 'approve' ? 'Approved' : 'Rejected'} ${ids.length} campaign(s)` 
    })
  } catch (error: any) {
